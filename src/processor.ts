...
export async function processMarkdownFile(
  ...
  if (description) {
    ...
    if (hasHTMLTags) {
      if (ignoreHTML) {
        // Skip this file if ignoreHTML is enabled and description contains HTML
        return null;
      }
    ...
    try {
      ...
      if (context.routeMap) {
        ...
        if (resolvedUrl && resolvedUrl !== `/${pathPrefix}/${relativePath}`) {
          console.log(
            `Resolved URL for ${path.basename(filePath)}: ${resolvedUrl} (was: /${pathPrefix}/${relativePath})`,
          );
        }
      }
      ...
      const docInfo = await processMarkdownFile(
        ...
        context.options.ignoreHTML || false,
        resolvedUrl,
      );
      if (docInfo !== null) {
      ...
      console.warn(`Error processing ${filePath}: ${message}`);
    }
  }
